...
open class CacheManager {
        ...
        if let fileName = encodedFileName(urlString: urlString),
           let url = fileURL(fileName: fileName) {
            if let data = try? Data(contentsOf: url),
               let dictionary = try? NSKeyedUnarchiver.unarchivedObject(ofClasses: [NSDictionary.self, NSString.self],
                                                                        from: data) {
                return dictionary
            } else {
    ...
    public func write(urlString: String, data: Any, header: ResponseHeader? = nil) throws {
        ...
        if let header = header {
            ...
            dataToWrite = try? NSKeyedArchiver.archivedData(withRootObject: dataAndHeader,
                                                            requiringSecureCoding: false)
        } else {
            dataToWrite = try? NSKeyedArchiver.archivedData(withRootObject: data,
                                                            requiringSecureCoding: false)
        }
